...
// Matches an incomplete opening tag that hasn't closed with > yet (still streaming attributes)
const INCOMPLETE_OPEN_TAG_RE = /<antArtifact(?:\s[^>]*)?$/
...
export function extractTagArtifacts(text: string): ExtractResult {
  ...
  if (partialMatch) {
    ...
    // 3. Check for incomplete opening tag (still streaming attributes, no > yet)
    const incompleteMatch = cleanedText.match(INCOMPLETE_OPEN_TAG_RE)
    if (incompleteMatch) {
      hasStreamingArtifact = true
      // Try to extract a title from the partial attributes
      const partialAttrs = parseAttributes(incompleteMatch[0])
      const title = partialAttrs.title || 'Generating...'
      const placeholder = `${PLACEHOLDER_PREFIX}${title}${PLACEHOLDER_SUFFIX}`
      cleanedText = cleanedText.replace(incompleteMatch[0], placeholder)
    }
  }
...
export function segmentMessageText(text: string): SegmentResult {
  ...
  if (partialMatch) {
    ...
    // 4. Check for incomplete opening tag (attributes still streaming, no > yet)
    const incompleteMatch = remaining.match(INCOMPLETE_OPEN_TAG_RE)
    if (incompleteMatch) {
      const beforeIncomplete = remaining.slice(0, remaining.indexOf(incompleteMatch[0]))
      if (beforeIncomplete.trim()) segments.push({ type: 'text', content: beforeIncomplete })

      const partialAttrs = parseAttributes(incompleteMatch[0])
      const artifact = tagToArtifact({
        identifier: partialAttrs.identifier || `artifact-${artifactCount++}`,
        type: partialAttrs.type || 'text/html',
        title: partialAttrs.title || 'Generating...',
        content: '',
        isStreaming: true,
      })
      segments.push({ type: 'artifact', artifact, isStreaming: true })
