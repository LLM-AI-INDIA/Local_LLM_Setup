    ...
    const response = await fetch(connection.serverUrl, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: { tools: {} },
          clientInfo: { name: 'california-dmv', version: '1.0.0' },
        },
      }),
...
export async function getMcpTools(connectionId: string): Promise<McpTool[]> {
  ...
  // Return cached tools (stored as JSON in PostgreSQL)
...
/**
 * Format tool results in a readable way for LLMs (especially local models).
 * Converts large JSON objects into structured text summaries.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function formatToolResultForLLM(output: any, toolName: string): string {
  try {
    // If output has an error field, return it prominently
    if (output.error) {
    ...
    // Extract top-level summary fields (non-array, non-object)
    for (const [key, value] of Object.entries(output)) {
      if (key === 'records' || key === 'results' || key === 'files' || key === 'slow_requests') continue;
      if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
        // Nested object — flatten it
        const nested = Object.entries(value).map(([k, v]) => `  ${k}: ${v}`).join('\n');
        parts.push(`${key}:\n${nested}`);
    ...
    // Handle record arrays — show first few rows as a summary
    const recordKey = ['records', 'results', 'files', 'slow_requests'].find(k => Array.isArray(output[k]));
    if (recordKey) {
      const records = output[recordKey] as Record<string, unknown>[];
      const total = records.length;
      const preview = records.slice(0, 10); // Show max 10 rows

      if (preview.length > 0) {
        parts.push(`\n${recordKey} (showing ${preview.length} of ${total}):`);
        ...
        if (total > 10) {
          parts.push(`  ... and ${total - 10} more records`);
        }
      }
    }
  ...
  for (const mcpTool of mcpTools) {
    ...
    aiTools[toolKey] = tool({
      ...
      execute: async (args) => {
        ...
        const missingParams = required.filter(p => args[p] === undefined || args[p] === null || args[p] === '');
        if (missingParams.length > 0) {
          const paramHelp = required.map(p => {
            const prop = mcpTool.inputSchema?.properties?.[p] as { type?: string; description?: string } | undefined;
            return `${p} (${prop?.type || 'string'}): ${prop?.description || 'required'}`;
          }).join(', ');
          console.error(`[MCP] Missing required parameters: ${missingParams.join(', ')}`);
          console.log(`[MCP] ========== TOOL EXECUTION END (VALIDATION) ==========`);
          return `ERROR: Missing required parameter(s): ${missingParams.join(', ')}. This tool requires: ${paramHelp}`;
        }
        ...
        try {
          ...
          if (result.isError) {
            ...
            return `ERROR: ${textContent || 'MCP tool execution failed'}`;
          }

          // Handle empty results
          if (!textContent || textContent.trim() === '') {
            console.log(`[MCP] Tool ${mcpTool.name} returned empty result`);
            console.log(`[MCP] ========== TOOL EXECUTION END (EMPTY) ==========`);
          ...
          // Try to parse as JSON first, otherwise return as plain text string
          try {
          ...
          return `ERROR: Tool execution failed: ${errorMsg}`;
        }
      },
      toModelOutput: async ({ output }) => {
        ...
        if (typeof output === 'string') {
          ...
          // Format JSON results more readably for local LLMs
          formattedResult = formatToolResultForLLM(output, toolName);
        }

        // Prefix with instruction to force the LLM to use the data
        const value = `[TOOL RESULT from ${toolName}]\n${formattedResult}\n[END TOOL RESULT]\n\nIMPORTANT: You MUST now present the above data to the user in a clear, professional summary. Do NOT call another tool unless the user asks for more data.`;
  ...
  for (const connectionId of activeMcpIds) {
    ...
    try {
      ...
      if (mcpTools.length > 0) {
        ...
        // Collect descriptions with parameter info for the system prompt
        for (const mcpTool of mcpTools) {
          const safeName = mcpTool.name.replace(/[^a-zA-Z0-9_]/g, '_');

          // Build parameter description to help LLM pass correct arguments
          let paramInfo = '';
          if (mcpTool.inputSchema?.properties) {
            const required = mcpTool.inputSchema.required || [];
            const params = Object.entries(mcpTool.inputSchema.properties).map(([key, schema]) => {
              const prop = schema as { type?: string; description?: string; enum?: string[] };
              const isRequired = required.includes(key);
              const enumVals = prop.enum ? ` (values: ${prop.enum.join(', ')})` : '';
              return `${key} (${prop.type || 'string'}${isRequired ? ', required' : ', optional'})${enumVals}${prop.description ? ': ' + prop.description : ''}`;
            });
            if (params.length > 0) {
              paramInfo = ` | Parameters: ${params.join('; ')}`;
            }
          } else {
            paramInfo = ' | Parameters: none';
          }

          allDescriptions.push({
            name: `mcp_${safeName}`,
            description: (mcpTool.description || `MCP tool: ${mcpTool.name}`) + paramInfo,
          });
        }
      }
    } catch (error) {
