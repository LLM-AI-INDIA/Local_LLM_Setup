...
import { vllm, isVLLMModel } from '@/lib/vllm';
...
import { buildSystemPromptWithTools, buildSystemPromptForLocal } from '@/lib/system-prompts';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const modelId = requestedModel || 'meta-llama/Llama-3.1-8B-Instruct';

    // Check if this is a local vLLM model
    const isLocalModel = isVLLMModel(modelId);

    // Check if reasoning should be enabled for this model
    const thinkingMode = (enableReasoning && !isLocalModel) ? getThinkingMode(modelId) : 'none';
    ...
    // Anthropic-specific tools (code execution, web search) — only for Claude models
    if (!isLocalModel) {
      // Code execution - always enabled for Claude
    ...
    const systemPrompt = isLocalModel
      ? buildSystemPromptForLocal(mcpToolDescriptions)
      : buildSystemPromptWithTools(toolNames, mcpToolDescriptions);
    ...
    const fittedMessages = fitMessagesToContextWindow(messages, systemPrompt, isLocalModel ? {
    ...
    const streamConfig: Record<string, any> = {
      model: isLocalModel ? vllm(modelId) : anthropic(modelId),
      system: systemPrompt,
      messages: fittedMessages,
      maxTokens: isLocalModel ? 4096 : 65536,
      ...(thinkingMode !== 'none' ? {} : { temperature: isLocalModel ? 0.3 : 0.7 }),
    ...
    if (hasTools) {
      ...
      // Local models (Llama etc.) — constrain tool behavior
      if (isLocalModel) {
        streamConfig.toolCallStreaming = false;
        streamConfig.toolChoice = 'auto';
        streamConfig.stopWhen = stepCountIs(50);
      }
    }

    // Build provider options
    if (isLocalModel) {
      // Disable parallel tool calls — Llama/vLLM only supports single tool calls per turn
    ...
    const stream = createUIMessageStream({
      ...
      onError: (error) => {
        ...
        if (error instanceof Error) {
          ...
          if (msg.includes('econnrefused') || msg.includes('fetch failed') || msg.includes('enotfound')) return 'The local LLaMA model is currently offline. The GPU may need to be scaled up.';
  ...
  const models = [
    {
      ...
      name: 'LLaMA 3.1 8B',
      description: 'Self-hosted open model (vLLM on AWS)',
